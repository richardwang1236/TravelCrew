...
def synthesizer_node(state: TravelState, config: Optional[RunnableConfig] = None) -> dict[str, Any]:
    ...
    system_prompt = (
        ...
        "For sections 7-10 where no structured data exists, generate reasonable suggestions based on "
        ...
        "- End each day with:\n"
        "  > 💰 Daily cost subtotal\n"
        "  > 💡 Evening suggestion (night view, local night market, rest early for next day)\n"
        "  > 🏨 Return to hotel: transport method & estimated time\n\n"

        "## Section 5: Food & Dining Guide\n"
        "Aggregate all dining entries from daily_itinerary into a categorized food guide:\n"
        "- 🍖 Signature Main Meals: list the must-try main restaurants with signature_dishes and per-person cost\n"
        "- 🥟 Local Snacks & Street Food: specific snack items, where to find them, price\n"
        "- ☕ Breakfast/Tea: morning food recommendations near the hotel area\n"
        ...
        "Suggest cultural/outdoor/specialty experience projects relevant to the destination.\n\n"
        ...
        "Provide three variants: time-tight condensed version, low-energy relaxed version, "
